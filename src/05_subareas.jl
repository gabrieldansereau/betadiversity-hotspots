...
left = -71.0; right = -64.0; bottom = 46.0; top = 50.0;
initial_coords = (left = left, right = right, bottom = bottom, top = top)
asp_ratio = 92.60/60.75
dim_ratio = (top-bottom)/(right-left)

# Function to increase subarea coordinates
function increase_subarea(initial_subarea::NamedTuple; leftlim = -145.0, bottomlim = 20.0, asp_ratio = asp_ratio, dim_ratio = dim_ratio)
    # Get initial coordinates
    left, right, bottom, top = initial_subarea
    # Increase left & bottom limits
    leftcoords = collect(left:-asp_ratio:leftlim)
    bottomcoords = collect(bottom:-(asp_ratio*dim_ratio):bottomlim)
    # Get smallest number of coordinates
    n = min(length(leftcoords), length(bottomcoords))
    # Arrange subarea coordinates s an Array of NamedTuples
    subareas_coords = [(left = leftcoords[i], right = right, bottom = bottomcoords[i], top = top) for i in 1:n]
    return subareas_coords
end
# Get coordinates
subarea_coords = increase_subarea(initial_coords, 
                                  leftlim = -145.0, bottomlim = 20.0,
                                  asp_ratio = asp_ratio, dim_ratio = dim_ratio
                                  )
    
# Alternative way to get analysis values
@time subarea_distributions = [map(d -> d[sc], distributions) for sc in subarea_coords]
@time subarea_Ys = calculate_Y.(subarea_distributions)
@time subarea_richness = [calculate_richness(Y, subarea_distributions[i][1]) for (i, Y) in enumerate(subarea_Ys)]
@time subarea_lcbd = [calculate_lcbd(Y, subarea_distributions[i][1]) for (i, Y) in enumerate(subarea_Ys)]

richness_medians = median.(subarea_richness)
lcbd_medians = median.(subarea_lcbd)
betadiv_values = calculate_BDtotal.(subarea_Ys)
gamma_values = calculate_gamma.(subarea_Ys)
