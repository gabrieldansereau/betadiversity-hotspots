...
env_df = CSV.read(joinpath("data", "proc", "distributions_env_full.csv"), DataFrame, header=true, delim="\t")
# Save new data
new = (distributions=distributions, spenames=spenames, Y=Y, inds_obs=inds_obs, Yobs=Yobs, env_df=env_df)
# Checkout a1cbac, re-run, & save old data
old = (distributions=distributions, spenames=spenames, Y=Y, inds_obs=inds_obs, Yobs=Yobs, env_df=env_df)
