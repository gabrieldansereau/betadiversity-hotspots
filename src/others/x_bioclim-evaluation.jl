...
@time df = CSV.read(
    joinpath("data", "proc", "ebd_warblers_prep.csv"); header=true, delim="\t"
)
# Separate species
...
@time distributions = @showprogress pmap(
    x -> bioclim(x, env_vars; training_layers=env_vars_train), train_sets[[24, 3]]
);
...
map_sp1 = plotSDM(
    distributions[1]; c=:BuPu, title="$(sp1) distribution", colorbar=:none, dpi=300
)
scatter!(
    map_sp1,
    [NaN];
