...
#### Compare richness
richness = DataFrame(;
    raw=richness_raw.grid[inds_pred_raw], sdm=richness_sdm.grid[inds_pred_raw]
)
comparison_richness = scatter(
    richness.raw,
    richness.sdm;
    markersize=1,
    title="Relationship between observed and predicted richness values",
    xlabel="Observed richness",
    ylabel="Predicted richness",
    legend=:none,
    grid=:none,
)
...
histogram(
    richness.raw;
    title="Distribution of observed richness",
    xlabel="Observed richness",
    ylabel="Count",
)
histogram(
    richness.sdm;
    title="Distribution of predicted richness",
    xlabel="Predicted richness",
    ylabel="Count",
)
...
histogram(
    richness.diff;
    title="Distribution of richness prediction offset",
    xlabel="Richness prediction offset",
    ylabel="Count",
)
scatter(
    richness.raw,
...
comparison_plot1 = scatter(
    lcbd.raw,
    lcbd.sdm;
    markersize=1,
    yticks=0.0:0.20:1.0,
    title="Relationship between observed and predicted LCBD values",
    xlabel="Observed LCBD",
    ylabel="Predicted LCBD",
    legend=:none,
    grid=:none,
)
...
histogram(
    lcbd.raw; title="Distribution of observed LCBDs", xlabel="Observed LCBD", ylabel="Count"
)
histogram(
    lcbd.sdm;
    title="Distribution of predicted LCBDs",
    xlabel="Predicted LCBD",
    ylabel="Count",
)
histogram(
    lcbd.diff;
    title="Distribution of LCBD prediction offset",
    xlabel="LCBD prediction offset",
    ylabel="Count",
)
